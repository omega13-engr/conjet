...
final class DaemonProcessSupervisorTests: XCTestCase {
    func testDefaultExpectedExecutableNameIsCanonicalConjetd() {
        let supervisor = DaemonProcessSupervisor(socketPath: "/tmp/conjetd.sock")

        XCTAssertEqual(supervisor.expectedExecutableNames, ["conjetd"])
    }
