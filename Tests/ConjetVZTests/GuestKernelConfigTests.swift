...
final class GuestKernelConfigTests: XCTestCase {
    ...
    func testConjetCoreImageScriptInstallsHVFReadinessMarkerUnit() throws {
        ...
        let storageScriptURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
            .appendingPathComponent("guest/image/conjet-core/scripts/conjet-docker-storage-setup.sh")
        let storageScript = try String(contentsOf: storageScriptURL, encoding: .utf8)

        let syntax = try ProcessRunner.run("/bin/bash", ["-n", imageScriptURL.path])
        XCTAssertTrue(syntax.succeeded, syntax.stderr)
        let storageSyntax = try ProcessRunner.run("/bin/sh", ["-n", storageScriptURL.path])
        XCTAssertTrue(storageSyntax.succeeded, storageSyntax.stderr)
        ...
        XCTAssertTrue(storageScript.contains("virtio-conjet-blk1"))
        XCTAssertTrue(storageScript.contains("security.capability"))
        XCTAssertTrue(storageScript.contains("CONFIG_SECURITY=y"))
        XCTAssertTrue(storageScript.contains("CONFIG_EXT4_FS_SECURITY=y"))
