...
struct ConjetBenchCLI {
    ...
    static func run(arguments: [String]) throws {
        ...
        case "startup-trace":
            try startupTraceBenchmark(args: args)
        case "help", "-h", "--help":
            printHelp()
        default:
    ...
    private static func startupTraceBenchmark(args: [String]) throws {
        let samples = max(1, value(after: "--samples", in: args).flatMap(Int.init) ?? 5)
        let conjetPath = expandedPath(
            value(after: "--conjet", in: args)
                ?? ProcessInfo.processInfo.environment["CONJET_BENCH_CONJET"]
                ?? ".build/debug/conjet"
        )
        let conjetHome = value(after: "--conjet-home", in: args).map(expandedPath)
        let outputDirectory = URL(
            fileURLWithPath: expandedPath(value(after: "--output-dir", in: args) ?? "benchmarks/reports/startup-trace-local"),
            isDirectory: true
        )
        let tracesDirectory = outputDirectory.appendingPathComponent("traces", isDirectory: true)
        let timeoutSeconds = value(after: "--command-timeout", in: args).flatMap(Double.init) ?? 30
        let maxExits = value(after: "--max-exits", in: args) ?? "16"
        let timeoutMilliseconds = value(after: "--timeout-ms", in: args) ?? "1000"
        let memoryMiB = value(after: "--memory-mib", in: args) ?? "512"

        try FileManager.default.createDirectory(at: tracesDirectory, withIntermediateDirectories: true)

        var results: [BenchmarkResult] = []
        for sample in 1...samples {
            let traceURL = tracesDirectory.appendingPathComponent("sample-\(sample).jsonl")
            var command = [
                conjetPath,
                "vm", "backend", "boot-attempt",
                "--max-exits", maxExits,
                "--timeout-ms", timeoutMilliseconds,
                "--memory-mib", memoryMiB,
                "--timeline-jsonl", traceURL.path,
                "--json"
            ]
            if let conjetHome {
                command.insert("CONJET_HOME=\(conjetHome)", at: 0)
                command.insert("/usr/bin/env", at: 0)
            }

            let startedAt = Date()
            let process = try runProcess(command, timeoutSeconds: timeoutSeconds)
            let duration = Date().timeIntervalSince(startedAt)
            var metrics = BenchmarkMetrics()
            metrics["sample"] = Double(sample)
            metrics.setString(traceURL.path, for: "startup_trace_jsonl")
            metrics.setString(conjetPath, for: "conjet_path")
            if let conjetHome {
                metrics.setString(conjetHome, for: "conjet_home")
            }

            if FileManager.default.fileExists(atPath: traceURL.path) {
                do {
                    let trace = try loadStartupTraceJSONL(traceURL)
                    try StartupTimelineValidator.validate(trace)
                    if let durationNanoseconds = trace.durationNanoseconds(from: .t0, to: .t7) {
                        metrics["t0_t7_ms"] = Double(durationNanoseconds) / 1_000_000.0
                    }
                    for event in trace.events {
                        metrics["\(event.id.rawValue.lowercased())_offset_ms"] = Double(event.offsetNanoseconds) / 1_000_000.0
                    }
                    metrics["startup_event_count"] = Double(trace.events.count)
                    metrics.setBool(trace.completeOrdered, for: "startup_trace_ordered")
                    if let t7 = trace.events.last(where: { $0.id == .t7 }) {
                        for (key, value) in t7.metrics {
                            metrics[key] = value
                        }
                    }
                } catch {
                    metrics.setBool(false, for: "startup_trace_ordered")
                    metrics.setString(String(describing: error), for: "startup_trace_error")
                }
            } else {
                metrics.setBool(false, for: "startup_trace_ordered")
                metrics.setString("trace file was not written", for: "startup_trace_error")
            }

            results.append(BenchmarkResult(
                workload: "jetstream-startup-trace",
                runtime: "conjet",
                command: command,
                startedAt: startedAt,
        ...
        let allResultsURL = outputDirectory.appendingPathComponent("all-results.json")
        try ConjetJSON.encoder().encode(results).write(to: allResultsURL, options: .atomic)
        let reportURL = outputDirectory.appendingPathComponent("startup-trace.md")
        try renderStartupTraceMarkdown(results).write(to: reportURL, atomically: true, encoding: .utf8)
        print("conjet-bench: startup-trace wrote \(allResultsURL.path)")
        print("conjet-bench: startup-trace wrote \(reportURL.path)")
    }

    private static func loadStartupTraceJSONL(_ url: URL) throws -> StartupTimelineTrace {
        let text = try String(contentsOf: url, encoding: .utf8)
        let events = try text
            .split(whereSeparator: \.isNewline)
            .map { line in
                try ConjetJSON.decoder().decode(StartupTimelineEvent.self, from: Data(String(line).utf8))
            }
        return StartupTimelineTrace(
            traceID: url.deletingPathExtension().lastPathComponent,
            label: "jetstream-startup-trace",
            startedAt: Date(timeIntervalSince1970: 0),
            startContinuousNanoseconds: events.first?.hostContinuousNanoseconds ?? 0,
            events: events
        )
    }

    private static func renderStartupTraceMarkdown(_ results: [BenchmarkResult]) -> String {
        let measured = results.filter { $0.exitCode == 0 || $0.metrics["t0_t7_ms"] != nil }
        let durations = measured.compactMap { $0.metrics["t0_t7_ms"] }.sorted()
        let failures = results.filter { $0.exitCode != 0 }.count
        return [
            "# Conjet Startup Trace Benchmark",
            "",
            "- Samples: \(results.count)",
            "- Failures: \(failures)",
            "- T0->T7 p50: \(formatMS(percentile(durations, 0.50))) ms",
            "- T0->T7 p95: \(formatMS(percentile(durations, 0.95))) ms",
            "",
            "| Sample | Exit | T0->T7 ms | Events | Ordered | Trace |",
            "|---:|---:|---:|---:|:---:|---|"
        ].joined(separator: "\n") + "\n" + results.enumerated().map { index, result in
            let ordered = result.metrics.value(for: "startup_trace_ordered")?.summaryValue ?? "false"
            let trace = result.metrics.value(for: "startup_trace_jsonl")?.summaryValue ?? ""
            return "| \(index + 1) | \(result.exitCode) | \(formatMS(result.metrics["t0_t7_ms"])) | \(formatMS(result.metrics["startup_event_count"])) | \(ordered) | `\(trace)` |"
        }.joined(separator: "\n") + "\n"
    }
    ...
    private static func printHelp() {
        print(
            ...
            Usage:
              ...
              conjet-bench startup-trace [options]
            ...
            Commands:
              ...
              startup-trace
                          Repeatedly run conjet vm backend boot-attempt and collect T0-T7 JSONL traces.
    ...
    private static func tail(_ text: String, limit: Int = 4096) -> String {
