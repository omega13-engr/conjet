...
public struct PolyglotBenchmarkSuite {
    ...
    private func prepareProject(for workload: String, at directory: URL) throws {
        ...
        case let name where name.hasPrefix("rust-"):
            try "cargo new --bin . >/dev/null 2>&1 || true\n".write(to: directory.appendingPathComponent(".prepared"), atomically: true, encoding: .utf8)
    ...
    private func imageAndScript(for workload: String) -> (image: String, script: String) {
        ...
        case "go-mod-download": return ("golang:1.23-alpine", "go mod download")
        case "go-build": return ("golang:1.23-alpine", "go mod download >/dev/null && go build ./...")
        case "go-test": return ("golang:1.23-alpine", "go mod download >/dev/null && go test ./...")
        case "rust-build": return ("rust:1-alpine", "cargo new --bin /tmp/poly >/dev/null && cd /tmp/poly && cargo build")
        case "rust-test": return ("rust:1-alpine", "cargo new --bin /tmp/poly >/dev/null && cd /tmp/poly && cargo test")
