...
struct NetworkMonitorView: View {
    @EnvironmentObject private var app: ConjetAppState
    @State private var selectedNetworkID: String? = "bridge"
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            ResourceSplitView {
                VStack(spacing: 0) {
                    PanelHeader(title: "Networks", subtitle: "1 total")
                    Divider()
                    List(selection: $selectedNetworkID) {
                        NetworkRow(
                            title: "bridge",
                            subtitle: "\(network?.activeTCPForwards ?? 0) TCP - \(network?.activeUDPForwards ?? 0) UDP",
                            tint: .teal
                        )
                        .tag("bridge")
